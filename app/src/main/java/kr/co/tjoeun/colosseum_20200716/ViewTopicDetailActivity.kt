...
//            투표를 안한 상태라면, 작성 화면 진입 거부 (투표부터 시키자)
            if (mTopic.mySideId == -1) {
                Toast.makeText(mContext,"투표를 해야 의견을 작성할 수 있다.",Toast.LENGTH_SHORT).show()
...
//            it => View형태 => 눌린 버튼을 담고있는 변수
            val clickedSideTag = it.tag.toString()
            Log.d("눌린 버튼의 태그",clickedSideTag)

//            눌린 버튼의 태그를 Int로 바꿔서 토론 주제의 진영중 어떤 진영을 눌렀는지 가져오는 index 로 활용
...
//                    이 응답에서, 토론 진행현황을 다시 파싱해서 화면에 반영영
